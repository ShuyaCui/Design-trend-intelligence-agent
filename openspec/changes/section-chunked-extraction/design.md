...
**决策**：若报告中找不到任何 H2/H3（chunk 数 < 2），则降级为现有行为（全文一次调用）。

**理由**：保证向后兼容，防止格式异常的报告完全无法提取。

### D3：Pass 2（风格）按 H2 切分提取，不按 H2/H3 细粒度 chunk 提取

**决策**：风格提取按 H2 标题切分（仅主章节，不含 H3 子章节），每个 H2 chunk 独立调用 `_STYLE_EXTRACTION_PROMPT` 一次；提取结果汇总后经 `_deduplicate_elements()` 合并。

**理由**：全文一次调用时 LLM 仍存在粒度压缩（每份报告只输出 2-3 个风格）。H2 粒度比 H2/H3 粗，保留了每个主题章节的完整上下文，适合风格这种"跨多个子章节但有主题边界"的维度。使用 H2/H3 细粒度会引入大量重复风格条目，去重成本高。

**备选**：全文一次提取 → 粒度压缩严重；H2/H3 细粒度 → 重复条目过多，同一风格在每个子章节各提取一次。
...
- **chunk 跨标题的上下文断裂**：有些报告在正文中引用前面章节的品牌案例。chunk 切分可能使 LLM 丢失该上下文。**缓解**：在每个 chunk 前拼接报告前 300 字（总览段落）作为 preamble。
