...
## Goals / Non-Goals
### Goals
- Replace embedding-only image retrieval with graph recall plus embedding reranking in the `attach_images` node.
...
- Do not add an embedding-only fallback when the KG returns no images.
...
- **Zero-result behavior**: If the KG returns no images for a material, return an empty list. This trusts the graph as the source of truth because Image nodes are already reported as having 100% coverage.
...
## Risks / Trade-offs
- Trusting KG recall improves precision but removes the broader recall of the previous embedding-only search; if graph edges are missing or stale, image results will be empty.
