...
### Requirement: Element-to-image vector retrieval
系统 SHALL 对每个推荐元素构造检索 query（`element_name + visual_keywords`），计算 query embedding，与图片 embedding 索引做 cosine similarity，返回 top-k 最相似的图片。

#### Scenario: Successful retrieval
- **WHEN** 推荐元素具有 `element_name` 和 `visual_keywords` 属性
- **THEN** 系统 SHALL 构造 query `"{element_name} {keyword1} {keyword2} ..."`，计算其 embedding，与 276 张图片的 embedding 做 cosine similarity，返回 similarity 最高的 top-3 图片的 `local_path` 和 `description`

#### Scenario: Empty keywords
- **WHEN** 推荐元素的 `visual_keywords` 为空列表
- **THEN** 系统 SHALL 仅使用 `element_name` 作为检索 query

#### Scenario: Multiple elements match same image
- **WHEN** 多个推荐元素的 top-3 结果包含同一张图片
- **THEN** 系统 SHALL 允许重复，每个元素独立保留其检索结果

### Requirement: attach_images graph node
推荐 graph SHALL 在 `recommend` node 之后包含一个 `attach_images` node，该 node 读取 state 中的推荐结果，对每个 `ElementRecommendation` 执行向量检索，将匹配的图片写入 `reference_images` 字段。
